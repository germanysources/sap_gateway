...
protected section.
  ...
  methods AIRPORTVALUEHELP_CREATE_ENTITY
    importing
    ...
    exporting
      !ER_ENTITY type ZCL_ZFLIGHT_MODEL_MPC=>TS_AIRPORTVALUEHELP
    raising
      /IWBEP/CX_MGW_BUSI_EXCEPTION
      /IWBEP/CX_MGW_TECH_EXCEPTION .
  methods AIRPORTVALUEHELP_DELETE_ENTITY
    importing
      !IV_ENTITY_NAME type STRING
      !IV_ENTITY_SET_NAME type STRING
      !IV_SOURCE_NAME type STRING
    ...
    raising
      /IWBEP/CX_MGW_BUSI_EXCEPTION
      /IWBEP/CX_MGW_TECH_EXCEPTION .
  methods AIRPORTVALUEHELP_GET_ENTITY
    importing
      !IV_ENTITY_NAME type STRING
      !IV_ENTITY_SET_NAME type STRING
      !IV_SOURCE_NAME type STRING
    ...
    raising
      /IWBEP/CX_MGW_BUSI_EXCEPTION
      /IWBEP/CX_MGW_TECH_EXCEPTION .
  methods AIRPORTVALUEHELP_GET_ENTITYSET
    importing
      !IV_ENTITY_NAME type STRING
      !IV_ENTITY_SET_NAME type STRING
      !IV_SOURCE_NAME type STRING
    ...
    exporting
      !ET_ENTITYSET type ZCL_ZFLIGHT_MODEL_MPC=>TT_AIRPORTVALUEHELP
      !ES_RESPONSE_CONTEXT type /IWBEP/IF_MGW_APPL_SRV_RUNTIME=>TY_S_MGW_RESPONSE_CONTEXT
    raising
      /IWBEP/CX_MGW_BUSI_EXCEPTION
      /IWBEP/CX_MGW_TECH_EXCEPTION .
  methods AIRPORTVALUEHELP_UPDATE_ENTITY
    importing
      !IV_ENTITY_NAME type STRING
      !IV_ENTITY_SET_NAME type STRING
      !IV_SOURCE_NAME type STRING
...
*     Call the entity set generated method
    airportvaluehelp_create_entity(
         EXPORTING iv_entity_name     = iv_entity_name
                   iv_entity_set_name = iv_entity_set_name
                   iv_source_name     = iv_source_name
                   io_data_provider   = io_data_provider
                   ...
                   it_navigation_path = it_navigation_path
                   io_tech_request_context = io_tech_request_context
       	 IMPORTING er_entity          = airportvaluehelp_create_entity
    ).
*     Send specific entity data to the caller interfaces
    copy_data_to_ref(
      EXPORTING
...
*     Call the entity set generated method
          airportvaluehelp_get_entity(
               EXPORTING iv_entity_name     = iv_entity_name
                         ...
                         it_navigation_path = it_navigation_path
                         io_tech_request_context = io_tech_request_context
             	 IMPORTING er_entity          = airportvaluehelp_get_entity
                         es_response_context = es_response_context
          ).

        IF airportvaluehelp_get_entity IS NOT INITIAL.
*     Send specific entity data to the caller interface
          copy_data_to_ref(
            EXPORTING
              is_data = airportvaluehelp_get_entity
            CHANGING
              cr_data = er_entity
          ).
        ELSE.
...
*-------------------------------------------------------------------------*
   WHEN 'AirportValueHelpSet'.
*     Call the entity set generated method
      airportvaluehelp_get_entityset(
        EXPORTING
         iv_entity_name = iv_entity_name
         iv_entity_set_name = iv_entity_set_name
         iv_source_name = iv_source_name
         ...
         io_tech_request_context = io_tech_request_context
       IMPORTING
         et_entityset = airportvaluehelp_get_entityset
         es_response_context = es_response_context
       ).
*     Send specific entity data to the caller interface
      copy_data_to_ref(
        EXPORTING
          is_data = airportvaluehelp_get_entityset
        CHANGING
          cr_data = er_entityset
      ).
...
*-------------------------------------------------------------------------*
      WHEN 'AirportValueHelpSet'.
*     Call the entity set generated method
          airportvaluehelp_update_entity(
               EXPORTING iv_entity_name     = iv_entity_name
                         iv_entity_set_name = iv_entity_set_name
                         iv_source_name     = iv_source_name
                         io_data_provider   = io_data_provider
                         ...
                         it_navigation_path = it_navigation_path
                         io_tech_request_context = io_tech_request_context
             	 IMPORTING er_entity          = airportvaluehelp_update_entity
          ).
       IF airportvaluehelp_update_entity IS NOT INITIAL.
*     Send specific entity data to the caller interface
          copy_data_to_ref(
            EXPORTING
...
* Call to Search Help run time mechanism to get values
  ...
  method AIRPORTVALUEHELP_CREATE_ENTITY.
  ...
  method AIRPORTVALUEHELP_DELETE_ENTITY.
  RAISE EXCEPTION TYPE /iwbep/cx_mgw_not_impl_exc
    EXPORTING
      textid = /iwbep/cx_mgw_not_impl_exc=>method_not_implemented
      method = 'AIRPORTVALUEHELP_DELETE_ENTITY'.
  ...
  method AIRPORTVALUEHELP_GET_ENTITY.
*-------------------------------------------------------------
* Datendeklaration
*-------------------------------------------------------------
...
ls_selopt-sign = 'I'.
ls_selopt-option = 'EQ'.
ls_selopt-low = ls_converted_keys-airport.
...
*-------------------------------------------------------------
* Zurückgeg. Suchhilfeergebnisse der Aufruferschnittstelle zuordnen - nur zugeordn.Attr.
*-------------------------------------------------------------
...
ENDLOOP.
  ...
  method AIRPORTVALUEHELP_GET_ENTITYSET.
*-------------------------------------------------------------
* Datendeklaration
*-------------------------------------------------------------
...
ENDLOOP.
  ...
  method AIRPORTVALUEHELP_UPDATE_ENTITY.
  RAISE EXCEPTION TYPE /iwbep/cx_mgw_not_impl_exc
    EXPORTING
      textid = /iwbep/cx_mgw_not_impl_exc=>method_not_implemented
      method = 'AIRPORTVALUEHELP_UPDATE_ENTITY'.
